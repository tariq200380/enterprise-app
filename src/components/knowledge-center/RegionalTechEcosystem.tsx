  ...
  return (
    <section className="w-full py-12 sm:py-14 bg-[#F8FAFC] border-b border-[#E2E8F0]">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-center flex-wrap gap-2 mb-8">
          {regionalWires.map((tab) => (
            <label
              ...
              className={`regional-btn-${tab.id} inline-flex items-center gap-1.5 px-4 py-2 rounded-md text-xs font-bold transition-all duration-200 cursor-pointer select-none bg-white text-[#475569] border border-[#CBD5E1] hover:bg-[#F8FAFC] hover:text-[#0F172A]`}
            >
              <span>{tab.icon}</span>
              <span>{tab.name}</span>
            </label>
        ...
        {/* Regional Story Panes (Controlled by Pure CSS) */}
        <div>
          {regionalWires.map((story) => (
            <div
              key={story.id}
              className={`regional-pane-${story.id} hidden grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-6 lg:gap-8 items-center bg-white border border-[#E2E8F0] rounded-2xl p-6 sm:p-7 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]`}
            >
              {/* Visual Container */}
              ...
              <div className="flex flex-col justify-center">
                ...
                <div>
                  ...
                  >
                    <span>Read Original on {story.sourceName}</span>
                    <span>&rarr;</span>
                  </a>
                </div>
              </div>
